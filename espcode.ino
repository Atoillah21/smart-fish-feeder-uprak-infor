...
const char* TOPIC_SCHEDULE = "feed/schedule"; // ✅ TAMBAHAN

// Pins
const int PIN_TRIG = 5;
const int PIN_ECHO = 18;
const int PIN_SERVO = 13;
const int PIN_LED = 2; // LED onboard ESP32
...
FeedTime schedules[MAX_SCHEDULE];
int scheduleCount = 0;
bool alreadyFed[MAX_SCHEDULE];
...
bool mqttConnect() {
  ...
  if (mqtt.connect(MQTT_CLIENT_ID)) {
    ...
    mqtt.subscribe(TOPIC_SCHEDULE); // ✅ TAMBAHAN
    return true;
  } else {
    ...
    for (JsonObject obj : arr) {
      if (scheduleCount < MAX_SCHEDULE) {
        ...
        if (h >= 0 && h <= 23 && m >= 0 && m <= 59) {
          ...
          alreadyFed[scheduleCount] = false;
          scheduleCount++;
        }
      }
    }

    prefs.putBytes("schedule", schedules, sizeof(schedules));
...
void setup() {
  ...
  digitalWrite(PIN_LED, LOW); // awal mati
  ...
  prefs.getBytes("schedule", schedules, sizeof(schedules));
...
void loop() {
  ...
  for (int i = 0; i < scheduleCount; i++) {
    if (hh == schedules[i].hour &&
        mm == schedules[i].minute &&
        !alreadyFed[i]) {
